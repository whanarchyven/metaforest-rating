...
const whichStat: any = (stat: any) => {
  switch (stat) {
    ...
    case 'level':
      return 'linear-gradient(270deg, #CAE4F8 0%, #CDFF07 100%)'
    case'carrots':
      return 'linear-gradient(90deg, #F8EDCA 0%, #FFD707 0.01%, #FF9519 100%)'
  }
}
...
export const StatRequirementBar = ({ stat_value, stat_name }: statRequirementBar) => (
  <div style={{ background: `${whichStat(stat_name)}` }} className={'h-full w-full rounded-full flex items-center'}>
    ...
    <p className={'justify-self-end ml-2 text-black font-bold'}>{stat_value}</p>
  </div>
)
